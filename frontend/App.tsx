      ...
      try {
        ...
        try {
          ...
          if (models && models.length > 0) {
            ...
            // Set first enabled model as default if current model doesn't exist
            const enabledModels = mappedModels.filter(m => m.enabled);
            if (enabledModels.length > 0) {
              const modelExists = mappedModels.some(m => m.id === modelConfig.modelId);
              if (!modelExists) {
                setModelConfig(prev => ({
                  ...prev,
    ...
    <div className="flex h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 overflow-hidden font-sans transition-colors duration-200">
      ...
      `}>
        <ChatList
          ...
          canManageProviders={userInfo?.role === 'Admin'}
          userInfo={userInfo}
          onLogout={handleLogout}
        />
      </div>

      <div className="flex-1 flex flex-col relative min-w-0 bg-white dark:bg-slate-950/50 transition-colors duration-200">

        {/* Header */}
        <header className="h-14 flex items-center justify-between px-4 bg-transparent sticky top-0 z-10">
          <div className="flex items-center gap-3">
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 -ml-2 text-slate-500 rounded-lg md:hidden"><Menu size={20} /></button>
            <div className="flex flex-col">
              {currentMessages.length > 0 && (
                <div className="font-semibold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                  {activeSession?.title}
                  {modelConfig.enableSearch && (
                    <span className="text-[10px] text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 px-1.5 py-0.5 rounded-full border border-blue-100 dark:border-blue-800">
                      WEB
                    </span>
                  )}
                </div>
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')} className="p-2 rounded-lg text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors" title={t('common.theme')}>
              {theme === 'light' ? <Moon size={18} /> : <Sun size={18} />}
            </button>
            <button onClick={() => setIsSettingsOpen(!isSettingsOpen)} className={`p-2 rounded-lg transition-colors ${isSettingsOpen ? 'bg-slate-200 dark:bg-slate-800 text-slate-900 dark:text-white' : 'text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800'}`} title={t('common.settings')}>
              <Settings size={18} />
            </button>
          </div>
        </header>
              ...
              <div key={msg.id} className={`flex gap-4 max-w-4xl mx-auto group mb-8 ${msg.role === Role.USER ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`flex flex-col max-w-[85%] md:max-w-[75%] min-w-0`}>
                  <div className={`flex items-center gap-2 mb-1 px-1 text-xs text-slate-400 dark:text-slate-500 ${msg.role === Role.USER ? 'flex-row-reverse' : ''}`}>
                    <span className="font-medium text-slate-600 dark:text-slate-300">{msg.role === Role.USER ? 'You' : 'AI'}</span>
